...
class ListOrders extends \Jane\OpenApiRuntime\Client\BaseEndpoint implements \Jane\OpenApiRuntime\Client\Psr7HttplugEndpoint
{
    ...
    protected function getQueryOptionsResolver(): \Symfony\Component\OptionsResolver\OptionsResolver
    {
        ...
        $optionsResolver->setDefined(['page', 'includeNonCompletePayments', 'filterQuery', 'filterPaymentMethodId', 'filterShippingMethodId', 'ordersCreatedAfter', 'ordersCreatedBefore', 'statusFilter', 'sortBy', 'sortOrder', 'include']);
        $optionsResolver->setRequired([]);
        $optionsResolver->setDefaults([]);
        $optionsResolver->setAllowedTypes('page', ['int']);
        $optionsResolver->setAllowedTypes('includeNonCompletePayments', ['bool']);
        $optionsResolver->setAllowedTypes('filterQuery', ['string']);
        $optionsResolver->setAllowedTypes('filterPaymentMethodId', ['int']);
        $optionsResolver->setAllowedTypes('filterShippingMethodId', ['int']);
        $optionsResolver->setAllowedTypes('ordersCreatedAfter', ['string']);
        $optionsResolver->setAllowedTypes('ordersCreatedBefore', ['string']);
        $optionsResolver->setAllowedTypes('statusFilter', ['int']);
        $optionsResolver->setAllowedTypes('sortBy', ['string']);
        $optionsResolver->setAllowedTypes('sortOrder', ['string']);
     ...
     *
     * @return \Starweb\Api\Client\Model\OrderModelCollection|null
     */
    protected function transformResponseBody(string $body, int $status, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        if (200 === $status && 'application/json' === $contentType) {
            return $serializer->deserialize($body, 'Starweb\\Api\\Client\\Model\\OrderModelCollection', 'json');
        }
        if (400 === $status && 'application/json' === $contentType) {
            throw new \Starweb\Api\Client\Exception\ListOrdersBadRequestException($serializer->deserialize($body, 'Starweb\\Api\\Client\\Model\\ErrorModel', 'json'));
        }
    }
}
